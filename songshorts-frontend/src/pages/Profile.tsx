...
const BRAND_GRAD = 'linear-gradient(135deg, #D946EF 0%, #D946EF 50%, #D946EF 100%)';
...
export default function Profile({ session }: { session?: Session | null }) {
  ...
  if (loading) {
    return <div className="h-full flex items-center justify-center"><div className="w-8 h-8 border-4 border-[#D946EF] border-t-transparent rounded-full animate-spin"></div></div>;
  }
        ...
        <div className="relative z-10 w-32 h-32 flex-shrink-0 aspect-square rounded-full p-[3px] mb-4 shadow-2xl" style={{ background: BRAND_GRAD }}>
          <div className="relative w-full h-full rounded-full overflow-hidden border-4 border-[#16141a] bg-[#1a1a1a] flex items-center justify-center group">
            ...
            <label className={`absolute inset-0 bg-[#16141a]/60 flex flex-col items-center justify-center transition-opacity cursor-pointer ${isEditing ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
              {uploadingAvatar ? (
                <div className="w-6 h-6 border-2 border-[#D946EF] border-t-transparent rounded-full animate-spin"></div>
              ) : (
                <>
        ...
        <div className="relative z-10 flex flex-col items-center text-center w-full max-w-md pb-10">
          {isEditing ? (
            <div className="w-full flex flex-col gap-3 mb-6 bg-[#1e1b24] p-6 rounded-2xl border border-white/10 text-left">
              ...
              <div className="flex gap-2 mt-2">
                ...
                <button 
                  ...
                  className="flex-1 py-2 rounded-lg bg-[#D946EF] text-white font-bold hover:bg-[#000000]"
            ...
            <>
              ...
              {profile?.full_name && <p className="text-sm text-[#D946EF] font-bold mb-2">@{profile?.username}</p>}
              
              {profile?.bio && <p className="text-white/80 mb-4">{profile.bio}</p>}
              
              <div className="flex items-center gap-2 bg-[#D946EF]/20 text-[#D946EF] px-4 py-1.5 rounded-full mb-6 text-sm font-bold border border-[#D946EF]/30">
              ...
              <div className="flex gap-3">
                ...
                <button 
                  ...
                  className="px-6 py-2.5 rounded-full bg-[#D946EF] text-[#16141a] text-sm font-bold shadow-[0_4px_14px_0_rgba(217,70,239,0.2)] hover:shadow-[0_6px_20px_rgba(217,70,239,0.3)] transition-all"
                >
                  Sign Out
                </button>
              </div>
        ...
        <div className="relative z-10 w-full max-w-md px-4 pb-32">
          <div className="bg-[#1e1b24] border border-white/5 rounded-2xl p-6">
            <h2 className="text-lg font-black text-white mb-4 flex items-center gap-2">
              <span className="material-symbols-outlined text-[#D946EF]">shield_person</span>
              Security & Policy
            </h2>
            <div className="flex flex-col gap-4 text-sm text-white/70">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-white font-medium">tanushri</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-white font-medium">tanushri.palavalasa@gmail.com</p>
                </div>
              </div>
            </div>
          </div>
