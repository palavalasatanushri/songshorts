  ...
  const renderTrackCard = (track: Track, list: Track[]) => {
    ...
    return (
      <div key={track.id} className="flex-shrink-0 w-32 group relative rounded-xl overflow-hidden cursor-pointer" onClick={() => handlePlay(track, list)}>
        <div className="relative w-32 h-32 mb-2 rounded-xl overflow-hidden shadow-lg">
          ...
          <div className={`absolute inset-0 bg-transparent/40 flex items-center justify-center transition-opacity ${isPlaying ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
            {isPlaying ? (
              <div className="w-6 h-6 flex justify-between items-end">
                <div className="w-1.5 bg-[#D946EF] h-full animate-bounce" style={{ animationDelay: '0ms' }} />
                <div className="w-1.5 bg-[#D946EF] h-2/3 animate-bounce" style={{ animationDelay: '150ms' }} />
                <div className="w-1.5 bg-[#D946EF] h-full animate-bounce" style={{ animationDelay: '300ms' }} />
              </div>
            ) : (
          ...
          <div className="absolute top-2 right-2 flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
            <button 
               ...
               className="w-8 h-8 rounded-full bg-transparent/50 backdrop-blur-md flex items-center justify-center hover:bg-transparent/70 hover:text-[#D946EF] text-white"
            >
        ...
        <p className={`text-sm font-bold truncate ${isPlaying ? 'text-[#D946EF]' : 'text-white'}`}>{track.title}</p>
    ...
    <div className="inner-scroll h-full overflow-y-auto px-4 pt-4 pb-6 w-full max-w-4xl mx-auto flex flex-col gap-6" style={{ paddingBottom: '100px' }}>
      ...
      <div className="sticky top-0 pt-2 pb-2 bg-transparent z-40">
        <div className="relative w-full">
          ...
          <input
            className="w-full bg-[#110f14] border border-transparent rounded-full py-3.5 pl-12 pr-4 text-base text-white placeholder-white/35 transition-all duration-200 focus:border-[#D946EF] focus:outline-none focus:ring-2 focus:ring-[#D946EF]/25"
          ...
          {isSearching ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {searchResults.map(track => {
                const isPlaying = currentTrack?.id === track.id;
                return (
                  <div key={track.id} onClick={() => handlePlay(track, searchResults)} className={`flex items-center gap-3 p-2 rounded-xl cursor-pointer transition-colors group ${isPlaying ? 'bg-[#D946EF]/10' : 'hover:bg-white/5'}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      <p className={`text-sm font-bold truncate ${isPlaying ? 'text-[#D946EF]' : 'text-white group-hover:text-[#D946EF] transition-colors'}`}>{track.title}</p>
                      <p className="text-xs text-white/50 truncate">{track.artist}</p>
                    </div>
                    <div className="flex items-center opacity-0 group-hover:opacity-100 transition-opacity">
                      <button 
                         onClick={(e) => saveToLibrary(track, e)}
                         className="p-2 hover:text-[#D946EF] text-white/50"
                      >
