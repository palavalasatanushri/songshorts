...
const GRADIENT_PRESETS = [
  { id: 'og',   label: 'SongShorts',   css: 'linear-gradient(135deg, #D946EF 0%, #86198F 100%)' },
      ...
      >
          ...
          {TABS.map(t => (
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-full text-sm font-bold transition-all ${
                tab === t.id ? 'bg-[#D946EF] text-black shadow' : 'text-white/50 hover:text-white'
              }`}
            >
            ...
            {GRADIENT_PRESETS.map(preset => (
              <button
                ...
                className={`w-full aspect-square rounded-xl transition-all active:scale-95 ${
                  selected === preset.id ? 'ring-2 ring-[#D946EF] ring-offset-2 ring-offset-[#111] scale-95' : ''
                }`}
            ...
            {EMOJI_COVERS.map(em => (
              <button
                ...
                className={`w-full aspect-square text-3xl rounded-xl bg-[#1e1b24] flex items-center justify-center transition-all active:scale-95 ${
                  selected === em ? 'ring-2 ring-[#D946EF] ring-offset-2 ring-offset-[#111] scale-95' : 'hover:bg-white/10'
                }`}
              >
                {em}
              </button>
        ...
        {tab === 'photo' && (
          <div className="flex flex-col items-center gap-4">
            ...
            <button
              ...
              disabled={uploading}
              className="w-full py-4 rounded-2xl border-2 border-dashed border-white/20 text-white/60 font-semibold hover:border-[#D946EF]/60 hover:text-[#D946EF] transition-all flex flex-col items-center gap-2 active:scale-95"
            >
              {uploading ? (
                <div className="w-7 h-7 border-2 border-[#D946EF] border-t-transparent rounded-full animate-spin" />
              ) : (
        ...
        <div className="flex gap-3 pt-1">
          ...
          <button
            ...
            style={{ background: 'linear-gradient(135deg, #D946EF 0%, #86198F 100%)' }}
          >
            Save Cover
          </button>
        </div>
...
export default function Library() {
  ...
  const fetchData = async () => {
    ...
    const cachedLiked = localStorage.getItem('songshorts_liked_tracks');
    const cachedPlaylists = localStorage.getItem('songshorts_playlists');
    ...
    try {
      ...
      if (user) {
        ...
        if (likedData) {
          ...
          localStorage.setItem('songshorts_liked_tracks', JSON.stringify(likedData));
        }
        const { data: playlistsData } = await supabase.from('playlists').select('*').eq('user_id', user.id).order('created_at', { ascending: false });
        if (playlistsData) {
          setPlaylists(playlistsData);
          localStorage.setItem('songshorts_playlists', JSON.stringify(playlistsData));
        }
      }
    } catch (err) {
  ...
  const fetchPlaylistTracks = async (id: string) => {
    ...
    const cacheKey = `songshorts_playlist_${id}`;
    ...
    return (
      <div className="inner-scroll h-full overflow-y-auto w-full max-w-2xl mx-auto flex flex-col" style={{ paddingBottom: '120px' }}>
        ...
        <div
          ...
          style={{
            background: (() => {
              ...
              if (!c) return 'linear-gradient(135deg, #D946EF 0%, #86198F 100%)';
        ...
        <div className="px-4 flex flex-col gap-1">
          {loadingTracks ? (
            <div className="flex justify-center py-10"><div className="w-8 h-8 border-4 border-[#D946EF] border-t-transparent rounded-full animate-spin" /></div>
          ) : playlistTracks.length === 0 ? (
            ...
            playlistTracks.map((track, i) => {
              ...
              return (
                <div
                  ...
                  className={`flex items-center gap-3 p-2.5 rounded-xl cursor-pointer transition-colors group ${isActive ? 'bg-[#D946EF]/10' : 'hover:bg-white/5'}`}
                >
                  ...
                  <div className="relative w-11 h-11 rounded-lg overflow-hidden flex-shrink-0">
                    ...
                    {isActive && (
                      <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
                        <span className="material-symbols-outlined text-[#D946EF] text-base" style={{ fontVariationSettings: "'FILL' 1" }}>graphic_eq</span>
                      </div>
                    )}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className={`text-sm font-bold truncate ${isActive ? 'text-[#D946EF]' : 'text-white'}`}>{track.title}</p>
      ...
      <section className="flex flex-col gap-4">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="flex items-center gap-1 text-sm text-[#D946EF] font-bold bg-[#D946EF]/10 px-3 py-1.5 rounded-full hover:bg-[#D946EF]/20 transition-colors"
          >
          ...
          <div className="bg-[#1e1b24] border border-white/10 rounded-2xl p-4 flex flex-col gap-4">
            <div className="flex items-center gap-3">
              ...
              <input
                ...
                className="flex-1 bg-transparent text-white outline-none placeholder-white/30 text-base font-semibold py-1 border-b border-white/10 focus:border-[#D946EF] transition-colors"
                autoFocus
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
          ...
          >
            <div className="w-full aspect-square rounded-xl flex items-center justify-center mb-2 shadow-lg group-hover:scale-105 transition-transform border border-[#D946EF]/20 group-active:scale-95" style={{ background: 'linear-gradient(135deg, #D946EF 0%, #86198F 100%)' }}>
            ...
            <p className="text-xs text-[#D946EF]">{likedTracks.length} songs</p>
          </div>
