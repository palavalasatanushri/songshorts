...
const BRAND = '#D946EF'; // Banana yellow
...
export default function Layout() {
  ...
  return (
    <div className="w-full text-white overflow-hidden flex flex-col md:flex-row relative" style={{ height: '100dvh', background: '#0a0a0f' }}>
      ...
      <div className="ambient-orb w-96 h-96 bg-[#D946EF]/8 top-[-10%] left-[-5%]" />
          ...
          {NAV_ITEMS.map(item => (
            <NavLink
              ...
              className={({ isActive }) => clsx(
                ...
                isActive ? 'text-[#D946EF]' : 'text-white/35 hover:text-white/70'
              )}
            >
              {({ isActive }) => (
                <>
                  ...
                  {isActive && (
                    <span className="absolute top-0 left-1/2 -translate-x-1/2 w-6 h-0.5 rounded-full bg-[#D946EF]" />
                  )}
                  <span
                    ...
                    style={{
                      fontVariationSettings: isActive ? "'FILL' 1" : "'FILL' 0",
                      fontSize: '23px',
                      filter: isActive ? 'drop-shadow(0 0 6px rgba(217,70,239,0.6))' : 'none',
                    }}
                  >
                    {item.icon}
                  </span>
