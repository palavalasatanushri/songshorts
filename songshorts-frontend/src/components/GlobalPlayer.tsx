...
declare global { interface Window { _songshortsAudio?: HTMLAudioElement; } }
...
export default function GlobalPlayer() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('songshorts:add-to-playlist', handler);
    return () => window.removeEventListener('songshorts:add-to-playlist', handler);
  }, []);
  ...
  return (
    <>
      {/* Toast */}
      {toast && (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 z-[300] bg-[#D946EF] text-black font-bold text-sm px-5 py-2 rounded-full shadow-lg">
          {toast}
        </div>
      )}
            ...
            {userPlaylists.length === 0 ? (
              ...
              <div className="flex flex-col gap-2 max-h-56 overflow-y-auto inner-scroll">
                {userPlaylists.map(pl => (
                  ...
                  >
                    <span className="material-symbols-outlined text-[#D946EF] text-xl" style={{ fontVariationSettings: "'FILL' 1" }}>queue_music</span>
          ...
          <div className="relative z-10 flex flex-col h-full px-6 pt-safe-top pb-safe-bottom" style={{ paddingTop: 'max(env(safe-area-inset-top), 20px)', paddingBottom: 'max(env(safe-area-inset-bottom), 24px)' }}>
            ...
            <div className="mb-1">
              <div className="w-full relative h-5 flex items-center group cursor-pointer">
                ...
                <div className="absolute left-0 h-1 rounded-full bg-[#D946EF] pointer-events-none" style={{ width: `${pct}%` }} />
                <div
            ...
            <div className="flex items-center justify-between mt-2 mb-6">
              <button onClick={toggleShuffle} className={`p-2 transition-colors ${isShuffle ? 'text-[#D946EF]' : 'text-white/40'}`}>
              ...
              <button
                ...
                style={{ background: '#D946EF', boxShadow: '0 8px 32px rgba(210,234,124,0.4)' }}
              >
              ...
              <button onClick={toggleRepeatMode} className={`p-2 transition-colors ${repeatMode !== 'off' ? 'text-[#D946EF]' : 'text-white/40'}`}>
            ...
            >
              <div className="px-4 pt-3 pb-2 flex items-center gap-2 border-b border-white/5">
                <span className="material-symbols-outlined text-[#D946EF] text-base" style={{ fontVariationSettings: "'FILL' 1" }}>lyrics</span>
                <span className="text-white/70 text-xs font-bold uppercase tracking-widest">Lyrics</span>
                {lyricsLoading && (
                  <div className="w-3 h-3 border-2 border-[#D946EF] border-t-transparent rounded-full animate-spin ml-auto" />
                )}
              </div>
      ...
      >
        ...
        <div className="w-full relative h-3 flex items-center cursor-pointer" onClick={e => e.stopPropagation()}>
          ...
          <div className="absolute left-0 h-0.5 rounded-full bg-[#D946EF] pointer-events-none" style={{ width: `${pct}%` }} />
          <input
            type="range"
            min="0"
            max={duration || 100}
